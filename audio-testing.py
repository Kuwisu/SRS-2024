...
Sx_db = 10 * np.log10(np.fmax(Sx, 1e-5)) # range is limited to -50 dB
...
# # s = librosa.stft(y2, n_fft=512, hop_length=128, win_length=512, window='hann')
# # s_db = librosa.amplitude_to_db(np.abs(s), ref=np.max)
# img = librosa.display.specshow(s_db, sr=16000, n_fft=512, hop_length=128, win_length=512,
#                                ax=ax[0], cmap='viridis', x_axis='time', y_axis='log')
# plt.colorbar(img, ax=ax[0])
#
# # Scipy spectrogram
# w = hann(512)
# SFT = ShortTimeFFT(win=w, hop=128, fs=16000, fft_mode='onesided', mfft=512)
# Sx = SFT.spectrogram(y2)
# Sx_db = 10 * np.log10(np.fmax(Sx, 1e-5)) # range is limited to -50 dB
# im1 = ax[1].imshow(Sx_db, origin='lower', aspect='auto', cmap='viridis',
#                    extent=SFT.extent(len(y2)))
